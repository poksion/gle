...
public final class WordDic {
	...
	try {
		...
		String key = line.substring(0, line.indexOf(delim,1));
		if(line.startsWith("\",\"")){
		    key = delim;
		}
	...
	try {
		...
		int p1 = s.indexOf(delim,1);     // key
		if(p1 == 1 && s.charAt(0) == '"' && s.length() > 3 && s.charAt(2) == '"'){
		    p1 = s.indexOf(delim,p1+1);
		}
...
//		System.out.println(data);
		...
		String key = s.substring(0, p1);
		if(key.startsWith("\"") && key.endsWith("\"")){
		    key = key.substring(1, key.length()-1);
		}
		final int id = wid.search(prefix+key);
		if(id < 0)
		    throw parseException("Word '"+key+"' is unregistered in trie",path,rl);
